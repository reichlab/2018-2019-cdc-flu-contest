...
stacking_model_fits_path <- "inst/estimation/region-stacking/fits"
...
for(target in as.character(unique(submission$Target))) {
  ...
  weight_names[weight_names == "ReichLab_KDE"] <- "kde"
  weight_names[weight_names == "ReichLab_KCDE"] <- "kcde"
  weight_names[weight_names == "ReichLab_SARIMA1"] <- "sarima_seasonal_difference_false"
  weight_names[weight_names == "ReichLab_SARIMA2"] <- "sarima_seasonal_difference_true"
