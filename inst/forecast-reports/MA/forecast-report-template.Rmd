...
#MA Results
What follows is the forecasted 1-4 week ahead ILI along with an 80% confidence interval around the forecasts. 
...
k_step_ahead_quantile <- c(get_ci_from_sample(model_csv[model_csv$Location=="Massachusetts" & model_csv$Target == "1 wk ahead" & model_csv$Type == "Bin",]$Value,.8),
...
upr <- k_step_ahead_quantile[seq(0,length(k_step_ahead_quantile),by=2)]
lwr <- k_step_ahead_quantile[seq(1,length(k_step_ahead_quantile),by=2)]
...
ggplot(data_ma[data_ma$season == "2018/2019",],aes(x=season_week,y=unweighted_ili)) + xlim(0,52)+ geom_line() +theme_bw() + xlab("Season Week") + ylab("ILI") + geom_vline(xintercept = tail(data$season_week,1),linetype="dashed") + geom_line(data=data.frame(y=c(tail(data_ma$unweighted_ili,1), k_step_ahead)),aes(x=(tail(data$season_week,1)):(tail(data$season_week,1)+4),y=y),col="cornflowerblue") +
      geom_ribbon(data=data.frame(lwr=lwr,upr=upr,y= k_step_ahead),aes(x=(tail(data$season_week,1)+1):(tail(data$season_week,1)+4),ymin=lwr,ymax=upr,y=y),alpha=0.3) + ggtitle("MA ILI")
